...
As vLLM models tend to be large, we will use the OCI feature (pre-built image with the model already included) to reduce startup time.

### Preparing the OCI container

First, make sure the OCI feature is enabled (it should be enabled by default):
...
...
        "enableModelcar": true
```
It will print additional content, the important part is `"enableModelcar": true`.
